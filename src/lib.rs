...
fn roll_type_parser(i: &str) -> nom::IResult<&str, Option<RollType>> {
    combinator::opt(branch::alt((
        combinator::value(RollType::WithAdvantage, bytes::complete::tag_no_case("a")),
        combinator::value(
...
fn dice_op_number_parser(i: &str) -> nom::IResult<&str, DiceRoll> {
    let result = sequence::tuple((dice_parser, operation_parser, character::complete::digit1))(i);
    match result {
...
fn statement_parser(i: &str) -> nom::IResult<&str, Vec<DiceRollWithOp>> {
    branch::alt((
        roll_op_statement_into_vec_roll_with_op,
        statement_into_roll_with_op, // roll_parser
    ))(i)
}

fn roll_op_statement_into_vec_roll_with_op(i: &str) -> nom::IResult<&str, Vec<DiceRollWithOp>> {
    let (remaining, (roll, operation, later_rolls)) =
        sequence::tuple((roll_parser, operation_parser, statement_parser))(i)?;
    let mut rolls = Vec::with_capacity(later_rolls.len() + 1);
    rolls.push(DiceRollWithOp::new(roll, Some(operation)));
...
fn statement_into_roll_with_op(i: &str) -> nom::IResult<&str, Vec<DiceRollWithOp>> {
    let (remaining, roll) = roll_parser(i)?;
    Ok((
        remaining,
        vec![DiceRollWithOp::new(roll, Some(Operation::Addition))],
    ))
}
...
pub fn parse_line(i: &str) -> Result<Vec<DiceRollWithOp>, ParserError> {
    // let i =
    let whitespaceless = i.replace(" ", "");
    match multi::many1(statement_parser)(&whitespaceless) {
        Ok((remaining, rolls)) => {
            ...
            return Ok(rolls
                ...
                .collect::<Vec<DiceRollWithOp>>());
        }
        Err(Err::Error(e)) | Err(Err::Failure(e)) => {
            return Err(ParserError::ParseError(nom::error::convert_error(
                "Ahhh",
